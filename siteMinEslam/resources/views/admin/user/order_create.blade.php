    ...
    <section class="content">
        <div class="container">
            <div class="col-md-10 m-auto">
                ...
                <div class="row mb-10">
                    <div class="col-md-12">
                        <h2><?php if (isset($page_title) && $page_title == 'Edit Bill') {
                            echo 'Edit';
                        } else {
                            echo 'Create New Bill';
                        } ?>
                            <?php if (isset($page_title) && $page_title == 'Edit Bill') : ?>
                            <a href="<?php echo url('admin/bills/details/' . md5($invoice[0]['id'])); ?>" class="btn btn-default btn-rounded pull-right"><i
                                    class="fa fa-long-arrow-left"></i> <?php echo helper_trans('back'); ?></a>
                            <?php else : ?>
                            <a href="<?php echo url('admin/bills'); ?>" class="btn btn-default btn-rounded pull-right">
                                <?php echo helper_trans('bills'); ?></a>
                            <?php endif ?>
                        </h2>
                    </div>
                </div>

                <form id="order_form" method="post" enctype="multipart/form-data" class="validate-form leave_con"
                    action="<?php echo url('admin/bills/add'); ?>" role="form" novalidate>
                    @csrf
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="text" class="form-control text-right" name="title"
                                                        value="<?php echo helper_trans('bill'); ?> <?php echo get_invoice_number(3); ?>">
                                                    <?php endif ?>
                                                    <input type="text" id="example-input-large" name="summary"
                                                        class="form-control form-control-md text-right"
                                                        placeholder="Summary Example" value="<?php echo html_escape($invoice[0]['summary']); ?>"> <br>
                                                ...
                                                <div class="col-xs-6 col-md-8 text-right">
                                                    <div class="form-group row">
                                                        <label for="inputEmail3"
                                                            class="col-sm-8 text-right control-label col-form-label"><?php echo helper_trans('bill') . ' ' . helper_trans('number'); ?></label>
                                                        <div class="col-sm-4">
                                                            <?php if (isset($page_title) && $page_title == 'Edit Bill') : ?>
                                                            <input type="text" class="form-control" name="number"
                                                                value="<?php echo html_escape($invoice[0]['number']); ?>"
                                                                placeholder="<?php echo helper_trans('bill') . '' . helper_trans('number'); ?>">
                                                            <?php else : ?>
                                                            <input type="text" class="form-control" name="number"
                                                                value="<?php echo date('Y') . '-' . str_pad(get_invoice_number(3), 2, '0', STR_PAD_LEFT); ?>">
                                                            <?php endif ?>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group row">
                                                        <label for="inputEmail3"
                                                            class="col-sm-8 text-right control-label col-form-label"><?php echo helper_trans('date'); ?></label>
                                                        <div class="col-sm-4">
                    ...
                    <div class="row mb-20">
                        <div class="col-md-12 text-center p-20">
                            <button type="submit" class="btn btn-info btn-rounded save_bill_btn"><i
                                    class="fa fa-check"></i> <?php if (isset($page_title) && $page_title == 'Edit Bill') {
                                        echo helper_trans('update');
                                    } else {
                                        echo helper_trans('save') . ' ' . helper_trans('bill');
                                    } ?></button>
                        </div>
                    </div>
...
<div id="productModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="vcenter"
    ...
    <div class="modal-dialog modal-dialog-zoom modal-md">
        <form id="product-form" method="post" enctype="multipart/form-data" class="validate-form"
            action="<?php echo url('admin/invoice/ajax_add_product/buy'); ?>" role="form" novalidate>
            @csrf
...
<div id="customerModal" class="modal fade" role="dialog" aria-labelledby="vcenter" aria-hidden="true">
    <div class="modal-dialog modal-dialog-zoom modal-md">
        <form id="customer-form" method="post" enctype="multipart/form-data" class="validate-form"
            action="<?php echo url('admin/invoice/ajax_add_vendor'); ?>" role="form" novalidate>
            @csrf
            <div class="modal-content modal-md">
                <div class="modal-header">
                    <h4 class="modal-title" id="vcenter"><?php echo helper_trans('add-new-vendor'); ?></h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-info waves-effect pull-right"><?php echo helper_trans('add-vendor'); ?></button>
                </div>
            </div>
        </form>
        <!-- /.modal-content -->
