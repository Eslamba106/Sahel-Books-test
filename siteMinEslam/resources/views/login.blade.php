...
<head>
    ...
    <title>{{ html_escape($settings->site_name) }} - {{ helper_trans('login') }}</title>

    <link rel="stylesheet" href="{{ asset('assets/admin/css/bootstrap.min.css') }}">

    <!-- Bootstrap 4.0-->
    <link rel="stylesheet" href="{{ asset('assets/admin/css/bootstrap-extend.css') }}">
    <!-- Theme style -->
    ...
    <div class="auth-box">
        ...
        <div class="login-box">
            ...
            <div class="login-logo" data-aos="fade-up" data-aos-duration="300">
                ...
                <a href="<?php echo url(''); ?>"><img width="50%" class="circles" src="<?php echo url($settings->logo); ?>"></a><br>
                <?php endif ?>
            </div>
            ...
            <div id="login-area" class="login-box-body" data-aos="fade-up" data-aos-duration="400">
                <p class="login-box-msg"><?php echo helper_trans('sign-in'); ?></p>
                <form id="login-form" method="post" action="<?php echo url('auth/log'); ?>">
                    @csrf
                    <div class="form-group has-feedback">
                        <input type="text" name="user_name" class="form-control log"
                            placeholder="<?php echo helper_trans('username-or-email'); ?>">
                        <span class="ion ion-email form-control-feedback"></span>
                    </div>

                    <div class="form-group has-feedback">
                        <input type="password" name="password" class="form-control log"
                            placeholder="<?php echo helper_trans('password'); ?>">
                        <span class="ion ion-locked form-control-feedback"></span>
                        <a class="pull-right forgot_pass" href="#"><?php echo helper_trans('forgot-password'); ?></a>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 text-center">
                            <button type="submit"
                                class="btn btn-info btn-block signin_btn"><?php echo helper_trans('sign-in'); ?></button>
                            <a class="create" href="<?php echo url('register'); ?>"><?php echo helper_trans('create-account'); ?></a>
                        </div>
                    </div>
                </form>
                <!-- /.social-auth-links -->
            ...
            <div id="forgot-area" class="login-box-body" style="display: none;">
                <p class="login-box-msg"><?php echo helper_trans('recover-password'); ?></p>

                <form id="lost-form" method="post" action="<?php echo url('auth/forgot_password'); ?>">
                    @csrf
                    <div class="form-group has-feedback">
                        <input type="email" name="email" class="form-control log"
                            placeholder="<?php echo helper_trans('enter-your-email'); ?>">
                        <span class="ion ion-email form-control-feedback"></span>
                        <a class="pull-right back_login" href="#"><i class="fa fa-angle-left"></i>
                            <?php echo helper_trans('back'); ?></a>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 text-center">
                            <button type="submit"
                                class="btn btn-info btn-block margin-top-10 signin_btn"><?php echo helper_trans('submit'); ?></button>
                        </div>
                    </div>
                </form>
                <!-- /.social-auth-links -->
            ...
            $(document).on('submit', "#login-form", function() {
                ...
                $.post($('#login-form').attr('action'), $('#login-form').serialize(), function(json) {
                    if (json.st == 1) {
                        ...
                        $(".error").show().html('<i class="icon-exclamation"></i> ' +
                            msg_wrong_access);
                        $('#login_pass').val('');
                    } else if (json.st == 2) {
                        $(".signin_btn").prop('disabled', false);
