...
class InvoiceController extends Controller
{
    ...
    {
        $data = array();
        $config['base_url'] = url('admin/invoice/type/' . $status);
        $total_row = get_invoices_by_type(1, 0, 0, $status, $invoice = 1);
        ...
        $data['status'] = $status;
        $data['invoices'] = get_invoices_by_type(0, $config['per_page'], $page * $config['per_page'], $status, $invoice = 1);
        $data['customers'] = get_by_user('customers');
        $data['main_content'] = view('admin/user/invoices', $data)->render();
